...
/**
 * Wrapper for Android Camera API. Used by Mediastreamer to record video from
 * webcam. This file depends only on Android SDK >= 5
 */
public class AndroidVideoApi5JniWrapper {
	...
	static public int detectCameras(int[] indexes, int[] frontFacing,
		...
		for (AndroidCamera androidCamera : cameras) {
			if (nextIndex == 2) {
				Log.w("mediastreamer",
						"Returning only the 2 first cameras (increase buffer size to retrieve all)");
				break;
			}
	...
	public static void stopRecording(Object cam) {
		Log.d("mediastreamer5", "stopRecording(" + cam + ")");
		Camera camera = (Camera) cam;
	...
	protected static int[] selectNearestResolutionAvailableForCamera(int id,
		...
		Log.d("mediastreamer", supportedSizes.size()
		...
		try {
			...
			Log.d("mediastreamer", "resolution selection done (" + r[0] + ", "
					+ r[1] + ", " + r[2] + ")");
			return r;
		} catch (Exception exc) {
	...
	protected static void applyCameraParameters(Camera camera, int width,
		...
		if (supported != null) {
			...
			Log.d("mediastreamer",
					"Preview framerate set:" + params.getPreviewFrameRate());
		}
