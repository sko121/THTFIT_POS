...
public class AndroidVideoApi9JniWrapper {
	public final static String TAG = "AndroidVideoApi9JniWrapper";
	...
	static public int[] selectNearestResolutionAvailable(int cameraId,
		...
		Log.d("mediastreamer", "selectNearestResolutionAvailable: " + cameraId
				+ ", " + requestedW + "x" + requestedH);
		return AndroidVideoApi5JniWrapper
				.selectNearestResolutionAvailableForCamera(cameraId,
	...
	public static Object startRecording(int cameraId, int width, int height,
		...
		Log.d("mediastreamer", "startRecording(" + cameraId + ", " + width
				+ ", " + height + ", " + fps + ", " + rotation + ", "
				+ nativePtr + ")");
		//Log.d(TAG, "startRecording,Pid="+Process.myPid()+",Tid="+Thread.currentThread().getId());
		try {
	...
	private static void setCameraDisplayOrientation(int rotationDegrees,
		...
		try {
			...
			Log.e("mediastreamer", "Failed to execute: camera[" + camera
	...
	private static int[] findClosestEnclosingFpsRange(int expectedFps,
			List<int[]> fpsRanges) {
		Log.d("mediastreamer", "Searching for closest fps range from "
				+ expectedFps);
		...
		for (int[] curRange : fpsRanges) {
			...
			if (curMeasure < measure) {
				...
				measure = curMeasure;
				Log.d("mediastreamer", "a better range has been found: w="
						+ closestRange[0] + ",h=" + closestRange[1]);
			}
		}
		Log.d("mediastreamer", "The closest fps range is w=" + closestRange[0]
				+ ",h=" + closestRange[1]);
		return closestRange;
	}
}
