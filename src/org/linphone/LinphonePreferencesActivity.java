...
import static com.thtfit.pos.R.string.ec_calibrating;
...
import static com.thtfit.pos.R.string.pref_echo_cancellation_key;
import static com.thtfit.pos.R.string.pref_echo_canceller_calibration_key;
import static com.thtfit.pos.R.string.pref_echo_limiter_key;
import static com.thtfit.pos.R.string.pref_media_encryption_key;
...
public class LinphonePreferencesActivity extends PreferenceActivity implements
	...
	private void detectAudioCodec(int id, String mime, int rate, boolean hide) {
		boolean enable = LinphoneService.isReady()
				&& LinphoneManager.getLc().findPayloadType(mime, rate) != null;
	...
	private void detectVideoCodec(int id, String mime) {
		findPreference(id).setEnabled(
				LinphoneManager.getInstance().detectVideoCodec(mime));
	}

	private void createDynamicAccountsPreferences() {
		accounts = (PreferenceCategory) getPreferenceScreen().getPreference(
				ACCOUNTS_SETTINGS_ID);
		accounts.removeAll();
	...
	private void createAddAccountButton() {
		Preference addAccount = (Preference) getPreferenceScreen()
				.getPreference(ADD_ACCOUNT_SETTINGS_ID);
		addAccount
				.setOnPreferenceClickListener(new OnPreferenceClickListener() {
					public boolean onPreferenceClick(Preference preference) {
						addExtraAccountPreferencesButton(accounts, nbAccounts,
								true);
						Intent i = new Intent();
						i.putExtra("Account", nbAccounts);
						nbAccounts++;
						i.setClass(LinphonePreferencesActivity.this,
								LinphonePreferencesSIPAccountActivity.class);
	...
	private void addExtraAccountPreferencesButton(PreferenceCategory parent,
			final int n, boolean isNewAccount) {
		SharedPreferences prefs = getPreferenceManager().getSharedPreferences();
		if (isNewAccount) {
		...
		if (prefs.getString(keyUsername, null) == null) {
			...
			me.setTitle(prefs.getString(keyUsername, "") + "@"
					+ prefs.getString(keyDomain, ""));
		}
	...
	private void addWizardPreferenceButton() {
		Preference wizard = (Preference) getPreferenceScreen().getPreference(
				WIZARD_SETTINGS_ID);
	...
	protected Dialog onCreateDialog(int id) {
		if (id == WIZARD_ID) {
			/*AlertDialog.Builder builder = new AlertDialog.Builder(
					LinphonePreferencesActivity.this);
			LayoutInflater inflater = LayoutInflater
					.from(LinphonePreferencesActivity.this);
			View v = inflater.inflate(R.layout.wizard, null);
			builder.setView(v);

			final EditText username = (EditText) v
					.findViewById(R.id.wizardUsername);
			ImageView usernameOkIV = (ImageView) v
					.findViewById(R.id.wizardUsernameOk);
			addXMLRPCUsernameHandler(username, usernameOkIV);

			final EditText password = (EditText) v
					.findViewById(R.id.wizardPassword);
			EditText passwordConfirm = (EditText) v
					.findViewById(R.id.wizardPasswordConfirm);
			ImageView passwordOkIV = (ImageView) v
					.findViewById(R.id.wizardPasswordOk);
			...
			ImageView emailOkIV = (ImageView) v
					.findViewById(R.id.wizardEmailOk);
			...
			createAccount.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					createAccount(username.getText().toString(), password
			...
			check.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					...
					if (isAccountVerified(username)) {
						...
						SharedPreferences.Editor editor = prefs.edit();
						editor.putBoolean(
								getString(R.string.pref_activated_key)
										+ (nbAccounts - 1), true);
						editor.commit();
					} else {
	...
	static boolean isAccountVerified(String username) {
		try {
			...
			Object resultO = client.call("check_account_validated", "sip:"
		...
		try {
			...
			XMLRPCCallback listener = new XMLRPCCallback() {
				Runnable runNotOk = new Runnable() {
					public void run() {
						errorMessage
								.setText(R.string.wizard_username_unavailable);
						usernameOk = false;
						icon.setImageResource(R.drawable.notok);
	...
	private boolean isEmailCorrect(String email) {
		return email
				.matches("^[a-z0-9]+([_\\.-][a-z0-9]+)*@([a-z0-9]+([\\.-][a-z0-9]+)*)+\\.[a-z]{2,}$");
	}
	...
	private void createAccount(final String username, final String password,
		...
		try {
			...
			client.callAsync(listener, "create_account", username, password,
					email, suscribe ? 1 : 0);
		} catch (Exception ex) {
			runOnUiThread(runNotReachable);
		}
		...
		TextWatcher passwordListener = new TextWatcher() {
			...
			public void onTextChanged(CharSequence arg0, int arg1, int arg2,
				...
				if (isPasswordCorrect(field1.getText().toString())
					...
					if (isPasswordCorrect(field1.getText().toString())) {
						errorMessage
								.setText(R.string.wizard_passwords_unmatched);
					} else {
						errorMessage
								.setText(R.string.wizard_password_incorrect);
					}
					icon.setImageResource(R.drawable.notok);
				}
				createAccount.setEnabled(usernameOk && passwordOk && emailOk);
	...
	private void verifiyAccountsActivated() {
		...
		for (int i = 0; i < nbAccounts; i++) {
			...
			boolean createdByWizard = prefs.getBoolean(
					getString(R.string.pref_wizard_key) + key, false);
			boolean activated = prefs.getBoolean(
					getString(R.string.pref_activated_key) + key, true);
			if (createdByWizard && !activated) {
				// Check if account has been activated since
				activated = isAccountVerified(prefs.getString(
						getString(R.string.pref_username_key) + key, ""));
				if (activated) {
					SharedPreferences.Editor editor = prefs.edit();
					editor.putBoolean(getString(R.string.pref_activated_key)
							+ key, true);
					editor.commit();
				} else {
					showDialog(CONFIRM_ID);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!getResources().getBoolean(R.bool.hide_accounts)) {
			...
			// Hide category
			PreferenceCategory accounts = (PreferenceCategory) getPreferenceScreen()
					.getPreference(ACCOUNTS_SETTINGS_ID);
			accounts.removeAll();
			accounts.setLayoutResource(R.layout.hidden);
		}

		if (getResources().getBoolean(R.bool.hide_wizard)) {
			Preference wizard = (Preference) getPreferenceScreen()
					.getPreference(WIZARD_SETTINGS_ID);
		...
		if (!Version.hasNeon()) {
			...
			findPreference(R.string.pref_video_codec_h264_key)
					.setEnabled(false);
			findPreference(R.string.pref_video_codec_h264_key).setDefaultValue(
					false);
		}
		...
		if (getResources().getBoolean(R.bool.hide_camera_settings)) {
			...
			PreferenceCategory videoSettings = (PreferenceCategory) screen
					.getPreference(CAMERA_SETTINGS_ID);
	...
	private void doOnFirstLaunch() {
		/*manageCheckbox(R.string.pref_echo_limiter_key,
				!Hacks.hasBuiltInEchoCanceller(), true, false);
		prefs().edit().putBoolean(LinphoneActivity.PREF_FIRST_LAUNCH, false)
				.commit();*/
	}
	...
	private void initializeMediaEncryptionPreferences() {
		...
		if (!hasSrtp && !hasZrtp) {
			...
			mencEntryValues
					.add(getString(R.string.pref_media_encryption_key_none));
			if (hasSrtp) {
				mencEntries.add(getString(R.string.media_encryption_srtp));
				mencEntryValues
						.add(getString(R.string.pref_media_encryption_key_srtp));
			}
			if (hasZrtp) {
				mencEntries.add(getString(R.string.media_encryption_zrtp));
				mencEntryValues
						.add(getString(R.string.pref_media_encryption_key_zrtp));
			}
	...
	private synchronized void startEcCalibration() {
		try {
			...
			ecCalibratePref
					.getEditor()
					.putBoolean(getString(pref_echo_canceller_calibration_key),
							false).commit();
		...
		mHandler.post(new Runnable() {
			public void run() {
				if (status == EcCalibratorStatus.Done) {
					ecCalibratePref.setSummary(String.format(
	...
	private void manageCheckbox(int key, boolean value, boolean enabled,
			boolean hidden) {
