...
import com.thtfit.SmartTerminal.SmartHomeAvRecorder;
import org.linphone.BaseErrDef;

class LinphoneCallImpl implements LinphoneCall {
	public final static String TAG = "LinphoneCallImpl";
	protected final long nativePtr;
	boolean ownPtr = false;
	private SmartHomeAvRecorder mSHAvRecorder = new SmartHomeAvRecorder();
	...
	private native int setSHAvRecordObjDesc(final long nativePtr, final int iSHAvRecordObjDesc);
	...
	private native boolean setAuthenticationTokenVerified(long nativePtr,
			boolean verified);
	...
	public boolean isInConference() {
		LinphoneCallParamsImpl params = new LinphoneCallParamsImpl(
				getCurrentParamsCopy(nativePtr));
	...
	public int startRecording(final String strFilePath)
	{
		...
		try {
			...
			iRet = mSHAvRecorder.create();
			if(0 != iRet)
			{
				Log.e(TAG, "mSHAvRecorder.create err "+iRet);
				return iOutRet;
			}
			iRet = mSHAvRecorder.setSavedFilePath(strFilePath);
			if(0 != iRet)
			{
				Log.e(TAG, "mSHAvRecorder.setSavedFilePath err "+iRet);
				return iOutRet;
			}
			iRet = mSHAvRecorder.startRecording();
			if(0 != iRet)
			{
				Log.e(TAG, "mSHAvRecorder.startRecording err "+iRet);
				return iOutRet;
			}
			iRet = setSHAvRecordObjDesc(nativePtr, mSHAvRecorder.getSHAvRecorderDesc());
			if(0 != iRet)
			{
				Log.e(TAG, "setSHAvRecordObjDesc err "+iRet);
				return iOutRet;
			}
		} finally {
		}
	...
	public int stopRecording()
	{
		...
		try {
			...
			iRet = mSHAvRecorder.stopRecording();
			if(0 != iRet)
			{
				Log.e(TAG, "mSHAvRecorder.stopRecording err "+iRet);
				return iOutRet;
			}
			iRet = mSHAvRecorder.destroy();
			if(0 != iRet)
			{
				Log.e(TAG, "mSHAvRecorder.destroy err "+iRet);
				return iOutRet;
			}
		} finally {
		}
