...
public interface LinphoneFriend {
	/**
	 * Enum controlling behavior for incoming subscription request. Use by
	 * {@link LinphoneFriend#setIncSubscribePolicy()}
	 * 
	 */
	static class SubscribePolicy {
		...
		/**
		 * Does not automatically accept an incoming subscription request. This
		 * policy implies that a decision has to be taken for each incoming
		 * subscription request notified by
		 * {@link LinphoneCoreListener#newSubscriptionRequest(LinphoneCore, LinphoneFriend, String)}
		 */
		public final static SubscribePolicy SPWait = new SubscribePolicy(0,
				"SPWait");
		/**
		 * Rejects incoming subscription request.
		 */
		public final static SubscribePolicy SPDeny = new SubscribePolicy(1,
				"SPDeny");
		/**
		 * Automatically accepts a subscription request.
		 */
		public final static SubscribePolicy SPAccept = new SubscribePolicy(2,
				"SPAccept");
	 ...
	 * {@link #edit()} before doing any attempts to modify friend configuration
	 * (such as address or subscription policy and so on). Once the
	 * modifications are done, then the application must call {@link #done()} to
	 * commit the changes.
	 */
	void edit();
