...
public class LinphoneCoreFactoryImpl extends LinphoneCoreFactory {
	...
	static {
		//AvRecord
		loadOptionalLibrary("SmartHomeAvRecord");
		// FFMPEG (audio/video)
		...
		if (!hasNeonInCpuFeatures()) {
			try {
				...
				Log.w("linphone",
						"Failed to load no-neon liblinphone, loading neon liblinphone");
	...
	public LinphoneCore createLinphoneCore(LinphoneCoreListener listener,
		...
		try {
			return new LinphoneCoreImpl(listener, new File(userConfig),
					new File(factoryConfig), userdata);
		} catch (IOException e) {
