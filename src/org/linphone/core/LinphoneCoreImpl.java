...
class LinphoneCoreImpl implements LinphoneCore {
	...
	private final LinphoneCoreListener mListener; // to make sure to keep a
													// reference on this object
	private long nativePtr = 0;

	private native long newLinphoneCore(LinphoneCoreListener listener,
	...
	private native void setDefaultProxyConfig(long nativePtr,
			long proxyCfgNativePtr);

	private native int addProxyConfig(LinphoneProxyConfig jprtoxyCfg,
			long nativePtr, long proxyCfgNativePtr);
	...
	private native void declineCallWithBusy(long nativePtr, long call);//added by lxj 2013-09-04
	...
	private native void setNetworkStateReachable(long nativePtr,
			boolean isReachable);
	...
	private native Object inviteAddressWithParams(long nativePtrLc, long to,
			long nativePtrParam);
	...
	private native long findPayloadType(long nativePtr, String mime,
			int clockRate);

	private native int enablePayloadType(long nativePtr, long payloadType,
			boolean enable);
	...
	private native void setPresenceInfo(long nativePtr, int minute_away,
			String alternative_contact, int status);

	private native long createChatRoom(long nativePtr, String to);

	private native void enableVideo(long nativePtr, boolean vcap_enabled,
	...
	private native void setPreferredVideoSize(long nativePtr, int width,
			int heigth);
	...
	private native void setSignalingTransportPorts(long nativePtr, int udp,
			int tcp, int tls);
	...
	private native void setMediaEncryptionMandatory(long nativePtr,
			boolean yesno);

	LinphoneCoreImpl(LinphoneCoreListener listener, File userConfig,
			File factoryConfig, Object userdata) throws IOException {
		mListener = listener;
		nativePtr = newLinphoneCore(listener, userConfig.getCanonicalPath(),
				factoryConfig.getCanonicalPath(), userdata);
	}
	...
	public synchronized void setDefaultProxyConfig(LinphoneProxyConfig proxyCfg) {
		isValid();
		setDefaultProxyConfig(nativePtr,
	...
	public synchronized void addProxyConfig(LinphoneProxyConfig proxyCfg)
		...
		isValid();
		if (addProxyConfig(proxyCfg, nativePtr,
	...
	public synchronized void terminateCall(LinphoneCall aCall) {
		isValid();
		if (aCall != null)
			terminateCall(nativePtr, ((LinphoneCallImpl) aCall).nativePtr);
	}

	public synchronized void declineCallWithBusy(LinphoneCall aCall) {//added by lxj 2013-09-04
		isValid();
		if (aCall != null)
			declineCallWithBusy(nativePtr, ((LinphoneCallImpl) aCall).nativePtr);
	}
	...
	public synchronized LinphoneCall invite(LinphoneAddress to)
			throws LinphoneCoreException {
		LinphoneCall call = (LinphoneCall) inviteAddress(nativePtr,
	...
	public synchronized void enablePayloadType(PayloadType pt, boolean enable)
			throws LinphoneCoreException {
		isValid();
		if (enablePayloadType(nativePtr, ((PayloadTypeImpl) pt).nativePtr,
				enable) != 0) {
	...
	public synchronized void setPresenceInfo(int minute_away,
			String alternative_contact, OnlineStatus status) {
		setPresenceInfo(nativePtr, minute_away, alternative_contact,
				status.mValue);
	...
	public synchronized LinphoneCall inviteAddressWithParams(
		...
		LinphoneCall call = (LinphoneCall) inviteAddressWithParams(nativePtr,
				ptrDestination, ptrParams);
		if (call != null) {
			return call;
		} else {
	...
	public synchronized void startEchoCalibration(Object data)
			throws LinphoneCoreException {
	...
	public synchronized void setSignalingTransportPorts(Transports transports) {
		setSignalingTransportPorts(nativePtr, transports.udp, transports.tcp,
				transports.tls);
	}
	...
	public void setVideoDevice(int id) {
		Log.i("SettingActivity camera id :", id);
	...
	private native int transferCallToAnother(long nativePtr, long callPtr,
			long destPtr);
	...
	private native void tunnelAddServerAndMirror(long nativePtr, String host,
			int port, int mirror, int ms);
	...
	public synchronized void acceptCallWithParams(LinphoneCall aCall,
		...
		acceptCallWithParams(nativePtr, getCallPtr(aCall),
	...
	private native void setVideoPolicy(long nativePtr, boolean autoInitiate,
			boolean autoAccept);
