 ...
 * {@link LinphoneCoreFactory#createLinphoneCore(LinphoneCoreListener, String, String, Object)}
 * .
 * 
 */
...
public interface LinphoneCore {
	...
	static public class GlobalState {
		 ...
		 * Startup
		 */
		static public GlobalState GlobalStartup = new GlobalState(1,
				"GlobalStartup");
		/**
		 * On
		 */
		 ...
		 * Shutdown
		 */
		static public GlobalState GlobalShutdown = new GlobalState(3,
				"GlobalShutdown");
	...
	static public class FirewallPolicy {
		 ...
		 */
		static public FirewallPolicy NoFirewall = new FirewallPolicy(0,
				"NoFirewall");
		/**
		 * Use NAT address (discouraged)
		 */
		static public FirewallPolicy UseNatAddress = new FirewallPolicy(1,
				"UseNatAddress");
		/**
	...
	static public class EcCalibratorStatus {
		 ...
		 */
		static public EcCalibratorStatus InProgress = new EcCalibratorStatus(
				IN_PROGRESS_STATUS, "InProgress");
		/**
		 * Calibration done
		 */
		static public EcCalibratorStatus Done = new EcCalibratorStatus(
				DONE_STATUS, "Done");
		/**
		 * Calibration in progress
		 */
		static public EcCalibratorStatus Failed = new EcCalibratorStatus(
	...
	/**
	 * Add a proxy configuration. This will start registration on the proxy, if
	 ...
	 * {@link LinphoneProxyConfig}. Toggling it as default will make
	 * LinphoneCore use the identity associated with the proxy configuration in
	...
	/**
	 * get he default proxy configuration, that is the one used to determine the
	 * current identity.
	 * 
	...
	/**
	 * Adds authentication information to the LinphoneCore. <br>
	 * This information will be used during all SIP transacations that require
	 * authentication.
	 * 
	 * @param info
	 */
	...
	/**
	 * Build an address according to the current proxy config. In case
	 * destination is not a sip address, the default proxy domain is
	 * automatically appended
	 * 
	 * @param destination
	 * @return
	 * @throws If
	 *             no LinphoneAddress can be built from destination
	 */
	public LinphoneAddress interpretUrl(String destination)
			throws LinphoneCoreException;

	/**
	 * Starts a call given a destination. Internally calls
	 * {@link #interpretUrl(String)} then {@link #invite(LinphoneAddress)}.
	 * 
	 * @param uri
	 */
	...
	/**
	 * Initiates an outgoing call given a destination LinphoneAddress <br>
	 * The LinphoneAddress can be constructed directly using
	 * linphone_address_new(), or created by linphone_core_interpret_url(). The
	 * application doesn't own a reference to the returned LinphoneCall object.
	 * Use linphone_call_ref() to safely keep the LinphoneCall pointer valid
	...
	/**
	 * decline a call with busy.
	 * @param aCall to be decline
	 */
	public void declineCallWithBusy(LinphoneCall aCall);//added by lxj 2013-09-04
	/**
	...
	/**
	 * 
	 * @return Returns true if in incoming call is pending, ie waiting for being
	 *         answered or declined.
	 */
	public boolean isInComingInvitePending();

	/**
	 * Main loop function. It is crucial that your application call it
	 * periodically.
	 * 
	 * #iterate() performs various backgrounds tasks: <li>receiving of SIP
	 * messages <li>handles timers and timeout <li>performs registration to
	 * proxies <li>authentication retries The application MUST call this
	 * function from periodically, in its main loop. <br>
	 * Be careful that this function must be call from the same thread as other
	 * liblinphone methods. In not the case make sure all liblinphone calls are
	 ...
	 * {@link LinphoneCoreListener#inviteReceived(LinphoneCore, String)}
	 * listener. The application can later respond positively to the call using
	 * this method.
	 * 
	 * @throws LinphoneCoreException
	 */
	...
	/**
	 * This method is called by the application to notify the Linphone core
	 * library when network is reachable. Calling this method with true trigger
	 * Linphone to initiate a registration process for all proxy configuration
	 * with parameter register set to enable. This method disable the automatic
	 * registration mode. It means you must call this method after each network
	 * state changes
	 * 
	 * @param network
	 *            state
	...
	/**
	 * get playback level [0..100]; -1 if not cannot be determined
	 * 
	 * @return
	 */
	...
	/**
	 * Initiate a dtmf signal to the speaker if not in call. Sending of the DTMF
	 * is done in another function.
	 * 
	 * @param number
	 * @param duration
	 *            in ms , -1 for unlimited
	 */
	 ...
	 * @throws LinphoneCoreException
	 */
	void enablePayloadType(PayloadType pt, boolean enable)
			throws LinphoneCoreException;
	...
	/**
	 * Get echo limiter status (another method of doing echo suppressionn, more
	 * brute force)
	 * 
	...
	/**
	 * @param transports
	 *            used for signaling (TCP, UDP and TLS)
	 */
	...
	/**
	 * add a friend to the current buddy list, if subscription attribute is set,
	 * a SIP SUBSCRIBE message is sent.
	 * 
	 * @param lf
	 *            LinphoenFriend to add
	 * @throws LinphoneCoreException
	 */
	 ...
	 *            how long in away
	 * @param status
	 *            sip uri used to redirect call in state LinphoneStatusMoved
	 */
	void setPresenceInfo(int minute_away, String alternative_contact,
			OnlineStatus status);

	/**
	 * Create a new chat room for messaging from a sip uri like
	 * sip:joe@sip.linphone.org
	 * 
	 * @param to
	 *            destination address for messages
	 * 
	 ...
	 * #LinphoneCore to initiate future calls with video or not. The two boolean
	 * parameters indicate in which direction video is enabled. SettingActivity both to
	 * false disables video entirely.
	 * 
	 * @param vcap_enabled
	 *            indicates whether video capture is enabled
	 * @param display_enabled
	 ...
	 * 
	 * @param stun_server
	 *            Stun server address and port, such as stun.linphone.org or
	 ...
	 * 
	 * @param pol
	 *            one of the FirewallPolicy members.
	 **/
	...
	LinphoneCall inviteAddressWithParams(LinphoneAddress destination,
			LinphoneCallParams params) throws LinphoneCoreException;
	 ...
	 * 
	 * @param path
	 *            The file must be a wav 16bit linear. Local ring is disabled if
	 *            null
	 */
	 ...
	 * 
	 * @param path
	 *            path to file with multiple PEM certif or to folder with
	 *            multiple PEM files
	 */
	...
	/**
	 * Start an echo calibration of the sound devices, in order to find adequate
	 * settings for the echo canceler automatically. status is notified to
	 * {@link LinphoneCoreListener#ecCalibrationStatus(EcCalibratorStatus, int, Object)}
	 * 
	 * @param User
	 *            object
	 * @throws LinphoneCoreException
	...
	void transferCallToAnother(LinphoneCall callToTransfer,
	 ...
	 * which could result in an active call. Eg: don't start a new call if one
	 * is in outgoing ringing. Eg: don't merge to conference either as it could
	 * result in two active calls (conference and accepted call).
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @params menc: MediaEncryption.None, MediaEncryption.SRTP or
	 ...
	 * 
	 * @return MediaEncryption.None, MediaEncryption.SRTP or
	 *         MediaEncryption.ZRTP
	 */
	...
	/**
	 * @param path
	 *            path to music file played to remote side when on hold.
	 */
	 ...
	 * @param port
	 *            tunnel server tls port, recommended value is 443
	 * @param udpMirrorPort
	 *            remote port on the tunnel server side used to test udp
	 *            reachability
	 * @param roundTripDelay
	 *            udp packet round trip delay in ms considered as acceptable.
	 *            recommended value is 1000 ms
	 */
	void tunnelAddServerAndMirror(String host, int port, int udpMirrorPort,
			int roundTripDelay);
