...
public final class ContactHelper {
	...
	public static boolean testPhotoUri(ContentResolver resolver,
			Uri photoUriToTest, String photoCol) {
		Cursor cursor = resolver.query(photoUriToTest,
				new String[] { photoCol }, null, null, null);
	...
	private final boolean queryOldContactAPI() {
		...
		String[] projection = { android.provider.Contacts.Phones.PERSON_ID,
				android.provider.Contacts.Phones.DISPLAY_NAME };
		String selection = android.provider.Contacts.Phones.NUMBER_KEY + "="
				+ normalizedNumber;
		Cursor c = resolver.query(android.provider.Contacts.Phones.CONTENT_URI,
		...
		while (c.moveToNext()) {
			...
			Uri personUri = ContentUris.withAppendedId(
					android.provider.Contacts.People.CONTENT_URI, id);
			Uri potentialPictureUri = Uri.withAppendedPath(personUri,
					android.provider.Contacts.Photos.CONTENT_DIRECTORY);
			boolean valid = testPhotoUri(resolver, potentialPictureUri,
					android.provider.Contacts.Photos.DATA);
			if (valid) {
				displayName = c
						.getString(c
								.getColumnIndex(android.provider.Contacts.Phones.DISPLAY_NAME));
				foundPhotoUri = personUri; // hack (not returning pictureUri as
											// it crashes when reading from it)
				c.close();
				return true;
			}
		}
	...
	private class ContactHelperNew {
		...
		private final boolean checkPhotosUris(ContentResolver resolver,
				Cursor c, String idCol, String nameCol) {
			if (c == null)
				return false;
			while (c.moveToNext()) {
				...
				Cursor photoCursor = resolver.query(photoUri, projection, null,
						null, null);
		...
		private final boolean queryNewContactAPI() {
			...
			if (Version.sdkAboveOrEqual(Version.API09_GINGERBREAD_23)) {
				selection = new StringBuilder()
						...
						.append("'").toString();
				c = resolver.query(uri, projection, selection,
						new String[] { sipUri }, null);
				valid = checkPhotosUris(
						resolver,
						c,
						android.provider.ContactsContract.Data.CONTACT_ID,
						android.provider.ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME);
